...
class Actions extends Base {
   ...
   * @param {('positive'|'destructive'|'default')|(ActionStyles.primary|ActionStyles.danger|ActionStyles.default)} [style = ActionStyles.default] action style. -OR- one of ActionStyles' keys. is 'default' by default
  ...
  openUrl({
    ...
    id,
    tooltip,
    style = this.ActionStyles.default,
    isEnabled = true,
  }) {
    const action = {
      type: "Action.OpenUrl",
   ...
   * @param {String} [iconUrl] optional icon to be shown on the action in conjunction with the title.
   * @param {String} [id] unique id for the button
   * @param {('positive'|'destructive'|'default')|(ActionStyles.primary|ActionStyles.danger|ActionStyles.default)} [style = ActionStyles.default] action style. -OR- one of ActionStyles. is 'default' by default
   * @param {String} [tooltip] works on v1.5 and later, defines text that should be displayed to the end user as they hover the mouse over the action.
   * @param {Boolean} [isEnabled = true] works on v1.5 and later, determines whether the action should be enabled. default true
